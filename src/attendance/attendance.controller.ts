...
export class AttendanceController {
  ...
  @ApiParam({
    name: 'date',
    description:
      'The date to filter attendance records by (format: YYYY-MM-DD)',
    example: '2024-01-01',
  })
  @ApiResponse({
  ...
  async addAttendanceByFamily(
    ...
    return this.attendanceService.addAttendanceByFamily(
      familyId,
      addAttendanceByFamilyDto,
      req,
    );
  }
  ...
  async addGeneralAttendanceByForm(
    @Body() generalAttendance: AttendanceSummaryDto,
  ): Promise<{ message: string }> {
  ...
  @Permissions(Permission.ViewGeneralAttendance)
  @Get('/:date')
  async getAttendancesByDate(@Param('date') date: Date) {
