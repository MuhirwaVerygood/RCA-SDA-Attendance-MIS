...
export class FamiliesService {
  ...
  // Create a new family
  async createFamily(
    familyRequest: CreateFamilyDto,
    req: any,
  ): Promise<object> {
    const { father_email, mother_email, password, father, mother, familyName } =
      familyRequest;

    const adder = await this.userRepository.find({
    ...
    const motherAsMember = await this.memberRepository.create({
      name: father,
      class: familyRequest.father_class,
      family,
    });
    await this.memberRepository.save(motherAsMember);
    const fatherAsMember = await this.memberRepository.create({
      name: mother,
      class: familyRequest.mother_class,
      family,
    });
  ...
  async getAllFamilies(): Promise<Family[]> {
    const families = await this.familyRepository.find({
      relations: ['members', 'attendances'],
    });
    if (!families) {
      throw new NotFoundException('Not families found');
    }

    const familiesWithActiveMembers = await Promise.all(
      families.map(async (family) => {
        const activeMembers =
          await this.calculateActiveMembersForFamily(family);
        return {
          ...family,
          activeMembers,
        };
  ...
  private async calculateActiveMembersForFamily(
    family: Family,
  ): Promise<number> {
  ...
  async getFamily(req: any): Promise<object> {
    ...
    if (!user || !user.family) {
      throw new NotFoundException("No family found for this user plz");
    }

    const familyWanted = await this.familyRepository.findOne({
