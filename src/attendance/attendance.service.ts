...
export class AttendanceService {
  ...
  async getGroupedAttendances(): Promise<any> {
    ...
    const rawAttendances = await this.attendanceRepository
      .createQueryBuilder('attendance')
      .select([
        'attendance.date',
        'SUM(attendance.abanditswe) AS abanditswe',
    ...
    const enrichedData = rawAttendances.map((attendance) => {
      if (!attendance.attendance_date) {
        throw new Error(
          `Missing or invalid date in attendance record: ${JSON.stringify(attendance)}`,
        );
      }
    ...
    const groupedAttendances = enrichedData.reduce(
      (group: Record<string, any[]>, record) => {
  ...
  async addAttendanceByFamily(
    familyId: number,
    addAttendanceByFamilyDto: AddAttendanceByFamilyDto,
    req: any,
    ...
    if (user.family.id != familyId) {
      throw new BadRequestException(
        'You are not authorized to add attendance for this family',
      );
    }
  ...
  async addChurchAttendance(attendanceRequest: {
    attendances: any[];
    abashyitsi: number;
  }): Promise<{ message: string }> {
  ...
  async addGeneralAttendanceByForm(
    ...
    const totalChurchMembers = families.reduce(
  ...
  async getAttendancesByDate(
    date: Date,
  ): Promise<Attendance[] | AttendanceSummaryDto> {
  ...
  async getFamilyAttendances(req: any): Promise<any> {
    ...
    // Fetch raw attendance data for the family, grouped by date
    const rawAttendances = await this.attendanceRepository
      .createQueryBuilder('attendance')
      .select([
        'attendance.date',
      ...
      .where('attendance.familyId = :familyId', { familyId: user.family.id })
      .groupBy('attendance.date')
      .orderBy('attendance.date', 'DESC')
      .getRawMany();
    ...
    const groupedAttendances = enrichedData.reduce(
      (group: Record<string, any[]>, record) => {
        const sabbath = record.sabbathDate.toISOString(); // Use ISO string for consistent grouping
        group[sabbath] = group[sabbath] || [];
        group[sabbath].push(record);
        return group;
      },
  ...
  async getFamilyAttendanceByDate(req: any, date: Date): Promise<any> {
    // Fetch the logged-in user and their family
    const user = await this.userRepository.findOne({
      where: { id: req.user.id },
      relations: ['family'],
    });
    ...
    // Fetch attendance records for the family on the specified date
    const attendance = await this.attendanceRepository.findOne({
      where: {
        family: { id: user.family.id }, // Filter by the family ID
        date, // Filter by the specified date
      },
    });
