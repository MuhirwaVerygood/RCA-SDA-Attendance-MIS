...
export class FamiliesController {
  ...
  async createFamily(
    @Body() familyRequest: CreateFamilyDto,
    @Req() req,
  ): Promise<object> {
    return this.familiesService.createFamily(familyRequest, req);
  }
  ...
  @UseGuards(AccessTokenGuard)
  @Get('/mine')
  async getFamily(@Req() req ): Promise<object> {
    return this.familiesService.getFamily(req);
  }
  ...
  @ApiParam({
    name: 'id',
    description: 'Unique ID of the family to update',
    example: 1,
  })
  @ApiResponse({
    status: HttpStatus.OK,
  ...
  @ApiParam({
    name: 'id',
    description: 'Unique ID of the family to delete',
    example: 1,
  })
  @ApiResponse({
    status: HttpStatus.OK,
    ...
    schema: {
      type: 'object',
      properties: {
        message: { type: 'string', example: 'Family deleted successfully.' },
      },
    },
  })
  @ApiResponse({
